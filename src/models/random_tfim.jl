...
"""
    ...
    RandomTFIM(; L, seed, D = 1.0, Omega = 1.0, site = SiteType("Qubit"))
...
A seed fixes the couplings and nothing else, so the same seed gives the same
chain to every downstream representation — the MPO built here and a
Jordan-Wigner treatment of the same `J`, `h` are the same Hamiltonian, not
equivalent ones.

See [`rescale_disorder`](@ref) for the `D → αD` map at fixed realisation.
"""
...
function RandomTFIM(;
    ...
    L >= 2 || error("RandomTFIM: need at least 2 sites, got L = $L")
    D > 0 || error("RandomTFIM: disorder strength D must be positive, got $D")
    rng = seed isa AbstractRNG ? seed : Xoshiro(seed)
    J = Omega .* rand(rng, L - 1) .^ D
    h = Omega .* rand(rng, L) .^ D
    return RandomTFIM(J, h; site=site)
end

"""
    nsites(m::RandomTFIM) -> Int

Chain length `L`.
"""
nsites(m::RandomTFIM) = length(m.h)
...
This is the continuation step of adaptive DMRG
([doi_10.1103_PhysRevB.98.195115](@cite)): the couplings keep their order and
their random draw, only the spread of their logarithms is scaled. `α = 0` is the
clean chain `J = h = Ω`, `α = 1` is `m`, and criticality is preserved throughout
...
function rescale_disorder(m::RandomTFIM, α::Real)
    ...
    return RandomTFIM(m.J .^ α, m.h .^ α; site=m.site)
end
...
function _bond_index(m::RandomTFIM, i::Int, j::Int)
    ...
    1 <= i <= nsites(m) - 1 || error("RandomTFIM: bond $i out of range 1:$(nsites(m) - 1)")
    return i
end

function _site_index(m::RandomTFIM, k::Int)
    1 <= k <= nsites(m) || error("RandomTFIM: site $k out of range 1:$(nsites(m))")
    return k
end
